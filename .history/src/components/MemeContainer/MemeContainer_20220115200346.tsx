...
function MemeContainer() {
  ...
  function handleDragStart(e: any, textData:{tId: string,top:number,left:number}) {
  ...
  function handleDragEnd(e: any, tId: string) {
    ...
    const topDif = e.clientY- mouseClients.mouseY ;
    const leftDif = e.clientX - mouseClients.mouseX;
    updateTextField({})
    if (dragText != null) {
        ...
        >
          {selectedMeme?.texts.map((t) => (
            <p
              ...
              // onDrag={handleDrag}
              onDragStart={(e) => handleDragStart(e, {tId:t.id,top:t.top||0,left:t.left})}
