...
function MemeContainer() {
  ...
  function handleDragStart(e: any,tTd: string) {
    console.log(e.clientX);
    e.target.style.opacity = "0";
  }

  function handleDragEnd(e: any,tTd:string) {
    console.log("e.cleientX", e.clientX);
    let dragText = document.querySelector(`#${tId}`)
    if (dragged.current) {
      const left = dragged.current.getBoundingClientRect().left;
      console.log("dragged.current.style.left", left);
      dragged.current.style.left = `${
        e.clientX - dragged.current.offsetWidth
      }px`;
    }
