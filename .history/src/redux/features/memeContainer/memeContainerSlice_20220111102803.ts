...
export const memeContainerSlice = createSlice({
  ...
  reducers: {
    ...
    updateTextField: (state, action) => {
      ...
      const selectedMeme = state.memesList.find(meme=>meme.id === state.selectedMemeId);

      const selectedTextIndex = selectedMeme?.texts.findIndex(
        (text) => text.id === action.payload.textId
      );
      selectedMeme.texts[selectedTextIndex].value =
...
export const { addImageToMeme, addTextField, setSelectedMeme,updateTextField } =
