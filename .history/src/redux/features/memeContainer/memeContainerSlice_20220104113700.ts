...
const initialState: {selectedMeme:SingleMeme,memesList: SingleMeme[]} = {selectedMeme:emptyMeme,memesList:[]};
...
export const memeContainerSlice = createSlice({
  ...
  reducers: {
    ...
    addTextField: (state, action) => {
      // const foundIndex = findIndex(state.memesList, action);
      const selectedMemeIndex = state.memesList.findIndex(meme=>meme.id === state.selectedMeme.id)
