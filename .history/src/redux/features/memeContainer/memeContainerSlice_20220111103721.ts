...
export const memeContainerSlice = createSlice({
  ...
  reducers: {
    ...
    updateTextField: (state, action) => {
      ...
      const selectedTextIndex = selectedMeme?.texts.find(
        (text) => text.id === action.payload.textId
      );
    if(selectedTextIndex !==undefined&& selectedMeme){

      selectedMeme.texts[selectedTextIndex].value = action.payload.upDatedVal;
    }
