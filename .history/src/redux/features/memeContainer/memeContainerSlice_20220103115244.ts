...
const memeContainerSlice = createSlice({
    ...
    reducers:{
        ...
        addText(state,action){
            const memeId :string = action.payload.id;
            const foundIndex = state.findIndex(meme=>meme.id ===action.payload.id)
...
function findIndex(state:[],action:{payload:singleMeme}){
    const memeId:string = action.payload.id;
    const foundIndex = state.findIndex(meme:singleMeme=>meme.id === memeId);
    return foundIndex
}
